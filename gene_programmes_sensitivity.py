...
cnmf_k = 16; cnmf_dt = '0_1'; cnmf_factor_order = [f'F{i}' for i in [12,15,5,7,14,1,6,11,3,2,8,10,13,4,9,16]]
cnmf_weights = pd.read_table(
    '/rds/project/rds-Nl99R8pHODQ/multiomics/programmes/cnmf/wang_2025/wang_2025_magma_genes/' +
    f'wang_2025_magma_genes.spectra.k_{cnmf_k}.dt_{cnmf_dt}.consensus.txt', index_col = 0).T
...
full_k = 16; full_dt = '0_15'; full_factor_order = [f'F{i}' for i in [12,15,5,9,14,2,1,8,4,6,7,11,13,3,10,16]]
# cnmf_k = 10; cnmf_dt = '0_1'; factor_order = [f'F{i}' for i in [9,10,4,5,6,7,8,1,2,3]]
full_weights = pd.read_table(
    '/rds/project/rds-Nl99R8pHODQ/multiomics/programmes/cnmf/wang_2025/wang_2025_neocortex/' +
    f'wang_2025_neocortex.spectra.k_{full_k}.dt_{full_dt}.consensus.txt',index_col = 0).T
full_weights.columns = [f'F{i+1}' for i in range(full_k)]
n_overlapping_genes = full_weights.index.intersection(cnmf_weights.index).size
print(f'Number of overlapping genes between full (including non-coding) and main dataset: {n_overlapping_genes}')
...
# for test_weights,k in zip(sensitivity_weights, sensitivity_k):
test_weights = cnmf_weights; k = cnmf_k
linkage_matrix = linkage(test_weights.T, method = 'average', metric = 'correlation')
dendrogram_res = dendrogram(linkage_matrix, no_plot = True)
...
corr = pd.concat([test_weights.corrwith(full_weights[f'F{i+1}']).to_frame(name = f'F{i+1}') for i in range(cnmf_k)], axis = 1)
corr = corr.loc[:, full_factor_order].iloc[dendrogram_res['leaves'], :]
out_prefix = f'{output_dir}/full_corr_k{cnmf_k}_to_k{k}'
fig = plt.figure(figsize = (10,10))
sns.heatmap(corr, cmap = 'vlag', center = 0, yticklabels = True, xticklabels = True, square = True)
fig.savefig(f'{out_prefix}.pdf', bbox_inches = 'tight')
plt.close()
